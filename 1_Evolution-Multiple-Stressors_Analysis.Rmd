...
This R notebook contains the code used for the data analysis in the paper: *Rapid Evolution Generates Synergism Between Multiple Stressors and Complicates Ecological Restoration*. 

We are interested in the following questions: 

- Do interactions between stressors change over time due to evolution?
...
- *assay*: "2", "3" - two growth assays (replication). Not enough levels to be set as a random effect and not significant. 
- *Environment*: 1-8 - 8 different environments
- *Population*: 1-32 - 4 replicates of each evolved line, missing three populations that went extinct: 11 (Cu), 19 (NaCu), and 24 (NaTemp). 
- *evolline*: 1-8 - 8 evolved lines of rotifers. 
- *Replicate*: 1-12 - there were ~12 replicates per combination.  
- *Growth*: A measure of fitness. Growth rate (% per day)
...
There is pseudoreplication due to the nested data structure (replicates within populations) - a mixed effects modeling approach would work but finding a specific error distribution (GLMM approach) was quite challenging because growth has a bimodal distribution (some die, some grow). Instead, we carried out our analysis at the population level. The mean growth rate of populations becomes the response variable. This reduces our statistical power but allows us to carry out simple statistics that clearly explain our results.
...
```{r, echo = TRUE}
rot_data <- read.csv("Data/growth_data.csv", header = TRUE, 
                     stringsAsFactors=FALSE)

# create a vector representing interactions of evolline and environment
# and recode in another vector as numeric (from 1 to 64). 
# These have been ordered along the rows of the original 
# 64 panel plot by using the magic that is lex.order=TRUE 
...
- **Calculate means (and standard error) of populations and create new dataframe**
...
## The user-defined functions (hedges.2, hedges.3) are from the scripts folder
...
number <- c("SC", "ST", "CT", "SCT", "SC", "ST", "CT", "SCT")

treatment_F <- as.factor(treatment)

# Jitter to enhance visibility in plots (figure 1)
...
growth_trend_data <- tibble(stressors, treatment, treatment_J, treatment_F, 
                                ...
                                effect2, cilow2, cihigh2, number)

# convert to factor 
growth_trend_data$stressors <- factor(growth_trend_data$stressors, 
                                   levels = c("NaCu", "NaTemp", "CuTemp", "All"))

# export this dataframe to use for creating figures 
write.csv(growth_trend_data, "Data/Outputs/stressor_interactions.csv", row.names=F)
...
t.test(effect1 ~ treatment_F, equal.var = TRUE, paired=TRUE, data=growth_trend_data, alternative = "two.sided")
...
## paired t-test 
t.test(effect2 ~ treatment_F, equal.var = TRUE, paired=TRUE, data=growth_trend_data, alternative = "two.sided")
